---
title: Flutter Interview Questions - Complete Collection
description: Flutter Interview Questions - Complete Collection
---

# Flutter Interview Questions - Complete Collection

> **Note:**
  **Total Questions:** 100

  This comprehensive collection covers Flutter fundamentals, Dart language features, state management, architecture patterns, and advanced topics.
...
1. **createState()**: When we build a new StatefulWidget, this one calls `createState()` right away and this override method must exist.
2. **initState()**: It is the first method called after the Widget is created. This is our equivalent to `onCreate()` and `viewDidLoad()`
3. **didChangeDependencies()**: This method is called immediately after `initState()` on the first time the widget is built.
4. **build()**: This is called right after `didChangeDependencies()`. All the GUI is rendered here and will be called every single time the UI needs to be rendered.
5. **didUpdateWidget()**: It'll be called once the parent Widget did a change and needs to redraw the UI.
6. **deactivate()**: Framework calls this method whenever it removes this State object from the tree
7. **dispose()**: It is called when this object and its State are removed from the tree permanently and will never build again.
...
The pubspec.yaml file is used to define the dependencies of your Flutter project. This metadata information is written in the YAML language. This file can have the following fields: name, version, description, homepage, repository, documentation, dependencies, environment, and more.
...
Flutter plugin is the wrapper of the native code like android (Kotlin or Java) and iOS (Swift or Objective C). Flutter can do anything that a native application can through the use of Platform Channels and Message Passing. Flutter instructs the native iOS/Android code to act and returns the result to Dart.
...
### WidgetsApp
A convenience class that wraps several widgets that are commonly required for an application. One of the primary roles that WidgetsApp provides is binding the system back button to pop the Navigator or quitting the application.

### MaterialApp
A convenience widget that wraps several widgets that are commonly required for material design applications. It builds upon a WidgetsApp by adding material-design specific functionality, such as AnimatedTheme and GridPaper.
...
- **Hot Restart** will again remove your previous state and run the complete program.
...
1. The first is to refactor a large widget tree into smaller individual widgets, each with its build method.
...
**Flexible** is used to resize the widgets in rows and columns. It's mainly used to adjust the space of the different child widgets while keeping the relation with their parent widgets.
...
The Material Design language was created for any platform, not just Android. When you write a Material app in Flutter, it has the Material look and feels on all devices, even iOS. If you want your app to look like a standard iOS-styled app, you would use the Cupertino library. You can technically run a Cupertino app on either Android or iOS, but (due to licensing issues) Cupertino won't have the correct fonts on Android. For this reason, use an iOS-specific device when writing a Cupertino app.
...
**Responsive**: A responsive app has had its layout tuned for the available screen size. Often this means (for example), re-laying out the UI if the user resizes the window, or changes the device's orientation. This is especially necessary when the same app can run on a variety of devices, from a watch, phone, or tablet, to a laptop or desktop computer.
...
Usually, you'll want to use `double.infinity`, but it's not always possible. Some Widgets allow their children to be as big as they want to be (Column, ListView, OverflowBox...).
...
For example: In the development stage, we use Mockito and test plugins and SDK to write the test cases and to test the complete behavior of the app. These kinds of plugins and SDKs we include in dev_dependencies. So in the release apps, there is no need of adding these plugins or SDK support.
...
**Deactivate**: Called when this object is removed from the tree. deactivate is called when a widget may be dispose. But that is not guaranteed.
...
- **Model**: Stores the application data. Handles domain logic and communication with database/network layers.
- **View**: UI layer that provides visualization of data stored in the Model and offers interaction to the user.
- **Controller**: Establishes the relationship between the View and the Model. Contains core application logic.

### The Model—View—Presenter (MVP) Pattern

MVP pattern overcomes the challenges of MVC:
- **Model**: Stores data and handles domain logic and communication with database/network layers.
- **View**: UI layer that provides visualization and keeps track of user's actions to notify the Presenter.
- **Presenter**: Fetches data from the model and applies UI logic. Manages the state of the View.

### The Model — View — ViewModel (MVVM) Pattern

MVVM pattern solves drawbacks of MVP:
- **Model**: Responsible for abstraction of data sources. Works with ViewModel to get and save data.
- **View**: Informs the ViewModel about user's action. Observes the ViewModel.
- **ViewModel**: Exposes data streams relevant to the View. Serves as link between Model and View.
...
Debug includes debugging information in the compiled files (allowing easy debugging) while Release usually has optimizations enabled. As far as conditional compilation goes, they each define different symbols that can be checked in your program.
...
It is used to provide a default value when an expression evaluates to null.

```dart
String? name = null;
String userName = name ?? "Admin";
```
...
### The ?. operator (null-aware access operator)

It is used to access properties or methods of an object that may be null, without throwing a null reference error.
...
In this example, the `?.` operator checks if the student object is null. If it is not null, then the `sayHello()` method is called on the student object. If student is null, then nothing happens.

Overall, the `??` operator is used to provide a default value when a variable is null, while the `?.` operator is used to safely access properties or methods of an object that may be null.
...
The required parameter is a well-known old-style parameter that we are all familiar with.
...
These are parameters that can be passed in any order by passing the name of the parameter followed by the passed value.
...
- Events need to be delivered in the correct order and without missing any
...
- Intended for individual messages that can be handled one at a time
- Can be used for mouse events in a browser, for example
- You can start listening at any time and get events fired while you listen
...
It's important to note that keys should only be used when necessary, as they can have a performance impact on your app.
...
Both futures and streams are parts of Dart. The main similarity between them is that they are both used for asynchronous programming. Also, one of the main similarities is that both will return in the future (asynchronous). One difference is that Future only returns once. The stream will return again and again.
...
- SizeBox is a utility widget in Flutter that enforces a fixed size constraint on its child widget without any visual representation of its own.
- It allows you to specify the width and height of its child widget.
- It is used to enforce specific dimensions (width and height) on its child widget.
...
Ticker in Flutter is a refresh rate of our animation. It is a class that sends a signal at a regular interval, i.e., around 60 times per second. We can understand it with our watch, which ticks at regular intervals. At each tick, Ticker provides a callback method with the duration since the first ticks at each second, after it was started. Even if the tickers started at different times, they always synchronized automatically.
...
In OOP, an **interface** is something that enforces the deriving class to implement a set list of public fields and methods. But unlike other traditional programming languages like C# and JAVA, Dart does not have explicit interface types. Each class, by default, defines its interface composed of public fields and methods. So, every class can act as an interface in Dart.

`implements` keyword is to implement an interface. Also, a class can implement multiple interfaces. In OOP, inheritance implies sharing of behavior between classes. We can not share features with an interface. So, when we implement a class, we can not share its behavior. If you want to share the behavior across these two classes, you should use the `extends` keyword.

In OOP, a **mixin** is a class that contains methods for use by other classes. Unlike the interface and inheritance approach, a mixin doesn't have to be the parent class of those other classes. So a mixin neither imposes usage restriction nor forces type restriction.
...
Provider is a wrapper around InheritedWidget to make them easier to use and more reusable. This is probably the approach you should start with. The provider package is easy to understand and it doesn't use much code. It also uses concepts that are applicable in every other approach.

By using provider instead of manually writing InheritedWidget, you get:
- State management pattern and separate business logic from UI
...
1. Code obfuscation
2. Manage background snapshots
3. Stay up-to-date with your Flutter version
4. Use Flushing in-memory cache
5. Use local authentication
6. Use Secure Storage
7. Restrict network traffic
8. Use jail-breaking protection
...
Provider is not a must, but should be considered.
...
If you use InheritedWidget in a large application, build methods always rebuild the whole build method. But with Provider you have Consumer widget which can be very specific to control specific blocks of the build method, so you have more efficiency. Also listeners have less complexity than InheritedWidgets' (O(N) vs O(N²)).
...
Lastly, since you'll need different state-management patterns for different projects, one package-for-all scenario is invaluable.
...
Dart provides the facility to call class instances like a function. To make callable class, we need to implement a `call()` method in it.
...
### Implicitly
In this case, the parent class is called implicitly, when there is object creation of child class. Here we don't make use of the super constructor but when the child class constructor is invoked then it calls the default parent class constructor.

### Explicitly
If the parent constructor is default then we call it as followed in implicit super, but if it takes parameters then the superclass is invoked using the super keyword.
...
In Dart, most of the functions are named functions. We can also create a nameless function known as an anonymous function, lambda, or closure. In Dart we can assign an anonymous function to constants or variables, later we can access or retrieve the value of closure based on our requirements.

---

## 90) What is FFI in Dart?

Dart's FFI (foreign function interface) allows Flutter apps to make use of existing native libraries that expose a C API. Dart supports FFI on Android, iOS, Windows, macOS, and Linux. For the web, Dart supports JavaScript interop.
...
## 92) Difference between Equatable vs Freezed?

Equatable and Freezed are both Dart packages that aid in simplifying code related to object equality and immutability, but they serve different purposes and offer varying levels of functionality.

**Equatable** is a lightweight solution primarily focused on providing a standardized way to implement equality comparisons in Dart. It simplifies the process by allowing developers to specify which properties should be considered for equality, thereby reducing boilerplate code for overriding the `==` operator and `hashCode` method.

**Freezed** is a more comprehensive code generation package that not only handles equality comparisons but also creates immutable data classes. It provides a range of features including deep equality checks, support for union types (sealed classes), and automatic generation of utility methods such as `copyWith`, `toString`, and JSON serialization/deserialization.

In simple words:
- **Equatable** is "An abstract class that helps to implement equality without needing to explicitly override == and hashCode."
- **Freezed** is "Code generation for immutable classes that has a simple syntax/API without compromising on the features."

They both override `==` and `hashCode`, but after that, they are very different. Equatable is a mixin to give a consistent, easy `==` and `hashCode`. Freezed is a build system that can use annotations to make immutable classes.

---

## 93) Explain FittedBox Widget?
...
## 95) How to check for types in Dart? Or What is sound typing in Dart?
...
## 97) Describe some Performance best practices in code
...
- When using an AnimatedBuilder, avoid putting a subtree in the builder function that builds widgets that don't depend on the animation. Instead, build that part of the subtree once and pass it as a child to the AnimatedBuilder
- Avoid clipping in an animation. If possible, pre-clip the image before animating it
- Avoid using constructors with a concrete List of children (such as Column() or ListView()) if most of the children are not visible on screen
...
## 98) How to hide Android StatusBar in Flutter?
...
## 99) How do you detect the host platform from Dart code?
...
## 100) How to get .apk and .ipa files from flutter?
...
**Note**: `--release` is optional as it is by default. If you need debug build, just replace `--release` with `--debug`

You can find the released APK or IPA files from:
```
